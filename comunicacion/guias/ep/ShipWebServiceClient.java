...
//			ProcessShipmentReply reply = port.processShipment(request); // This is the call to the ship web service passing in a request object and returning a reply object
//			
//			reply.getCompletedShipmentDetail().getMasterTrackingId();
//			//
//			if (isResponseOk(reply.getHighestSeverity())) // check if the call was successful
//			{
//				writeServiceOutput(reply);
//				System.out.println("Traking id: " + reply.getCompletedShipmentDetail().getMasterTrackingId().getTrackingNumber());
//			}	
//
//			printNotifications(reply.getNotifications());
//
...
//		}
	...
	public String crearGuia(String pedido) {
		cliente = new Cliente(pedido);
		ProcessShipmentRequest request = buildRequest();
		//
		try {
			...
			return reply.getCompletedShipmentDetail().getMasterTrackingId().getTrackingNumber();
...
//	    requestedShipment.setSpecialServicesRequested(addShipmentSpecialServicesRequested()); 
	...
	private static Party addShipper(){
	    ...
	    shipperContact.setPersonName("Cloe");
	    shipperContact.setCompanyName("Cloe SA de CV");
	    shipperContact.setPhoneNumber("50009292");
	    Address shipperAddress = new Address();
	    shipperAddress.setStreetLines(new String[] {"Jalisco 1300 Nave 4"});
	    shipperAddress.setCity("Zapopan");
	    shipperAddress.setStateOrProvinceCode("MX");
	    shipperAddress.setPostalCode("45200");
	...
	private static Party addRecipient(){
	    ...
	    recipientContact.setPersonName(cliente.getName());
	    recipientContact.setCompanyName(cliente.getCompany());
	    recipientContact.setPhoneNumber(cliente.getPhone());
	    Address recipientAddress = new Address();
	    recipientAddress.setStreetLines(new String[] {cliente.getStreet()});
	    recipientAddress.setCity(cliente.getCity());
	    recipientAddress.setStateOrProvinceCode(cliente.getState());
	    recipientAddress.setPostalCode(cliente.getPostalCode());
	    recipientAddress.setCountryCode("MX");
	...
	private static RequestedPackageLineItem addRequestedPackageLineItem(){
		...
		requestedPackageLineItem.setWeight(addPackageWeight(1.0, WeightUnits.KG));
		requestedPackageLineItem.setDimensions(addPackageDimensions(5, 5, 5, LinearUnits.CM));
	...
	private static void saveLabelToFile(ShippingDocument shippingDocument, String trackingNumber) throws Exception {
		...
		for (int a=0; a < sdparts.length; a++) {
			...
			if (labelLocation == null) {
				labelLocation = "C:\\Users\\ivanm\\Documents\\FedexEtiquetas\\";
			}
